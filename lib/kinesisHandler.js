 ...
 *       "invokeIdentityArn": "arn:aws:iam::123456789012:role/devProductCatalogReaderWriter",
 *       "awsRegion": "us-west-2",
 *       "eventSourceARN": "arn:aws:kinesis:us-west-2:123456789012:stream/devRetailStream"
 *     },
 *     {
 *       "kinesis": {
 *         "kinesisSchemaVersion": "1.0",
...
function KinesisSynchronousHandler(eventSchema, moduleName, transformer) {
   ...
   */
  this.recordGenerator = function* (kinesisEvent, callback) {
  ...
  this.processKinesisEvent = function (kinesisEvent, context, callback) {
    try {
      ...
      if (
        ...
        this.it = this.recordGenerator(kinesisEvent, callback)
        this.it.next()
      } else {
        callback(`${constants.MODULE} for ${this.module} ${constants.METHOD_PROCESS_KINESIS_EVENT} - no records received.`)
      }
